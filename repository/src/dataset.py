...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_item(self, index):
        ...
        #25 value is setted because of sigma setting to 25

        #then medianblur is applied to median the colors and to avoid cluttering

        img_color_domain = cv.medianBlur(img, random_blur)
        K = self.km
        # K = np.random.randint(2, 6)

        #K Means clustering is then used
