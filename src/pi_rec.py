...
class PiRec():
    ...
    def refine(self, img_blur, edge):
        ...
        # Setting lanczos interpolation for edges

        edge = resize(edge, size, size, interp='lanczos')

        #Setting masking for edges

        #When an edge has sigmoid value lower than 69 --> It will be treated as 0

        edge[edge <= 69] = 0

        #When an edge has sigmoid value greater than 69 --> It will be treated as 1
        edge[edge > 69] = 255

        #to tensor will convert the numpy image to Torch.Cuda image.
